...
# def get_imagenet_loader(batch_size, num_workers, h5_path,shuffle=True):
#     if os.path.exists(h5_path):
#         h5_store = h5py.File(h5_path, 'r')
#         data = h5_store['data'][:] # 通过切片得到np数组
#         target = h5_store['target'][:]
#         h5_store.close()
#         print("^_^ data loaded successfully from :{}".format(h5_path))
#     else:
#         print("expected file:{} not found".format(h5_path))
#
#     data = torch.from_numpy(data)
#     target = torch.from_numpy(target)  # numpy转Tensor
#     dataset = ImageNetDataset(data, target)
#     del data,target
#     return DataLoader(dataset=dataset, num_workers=num_workers, drop_last=True, batch_size=batch_size,
#                   shuffle=shuffle)

def _generate_adv_file(attack_method,num_classes,epsilon,set_size):
    # load model
    model = torch.load(os.path.join("checkpoint", "resnet50_epoch_7.pth"))
    model = model.cuda()
    ...
    torch.manual_seed(0)
    test_adv = np.zeros([set_size,3,224,224])
    ...
    h5_store = h5py.File("data/test_ImageNet_"+str(set_size)+"_adv_" + str(attack_method) + "_" + str(epsilon) + ".h5", 'w')
...
if __name__ == '__main__':
    ...
    _generate_adv_file(attack_method=args.attack_method, num_classes=1000, epsilon=args.epsilon,set_size=args.test_samples)
